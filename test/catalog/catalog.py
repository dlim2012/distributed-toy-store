...
RESTOCK_INTERVAL = int(os.getenv("RESTOCK_INTERVAL", 1))
RESTOCK_INTERVAL = 0.5
    ...
    def Order(self, request, context):
        ...
        if request.product_name not in self.retriever.keys():
            ...
            if quantity >= request.quantity:
                ...
                print("(Buy Successful) %s: (before: %d) -> (after: %d)" %(request.product_name, quantity, quantity-request.quantity))
            ...
            # 4) Not enough quantity: but failed
                ...
                # Print the buy result
                print("(Buy Failed) %s: (remaining: %d) < (requested: %d)" %(request.product_name, quantity, request.quantity))
