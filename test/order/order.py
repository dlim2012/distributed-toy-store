...
class OrderServicer(order_pb2_grpc.OrderServicer):
    ...
    def __init__(self, catalog_client, log_file):
        ...
        if COMPONENT_ID == 3 and len(self.log.keys()) == 1:
            print("############################## TEST ###############################")
            print('[Testcase 1] Filling missing logs.')

            # Send requests for missing logs
            self.threadpool.submit(self._missing_logs)
            sleep(0.5)
            print("###################################################################")
        elif COMPONENT_ID == 1 and len(self.log.keys()) == 4:
            print("############################## TEST ###############################")
            print('[Testcase 11] Filling missing logs.')
        ...
        for recovery_stub in self.recovery_stubs.values():
            if order_numbers == None:
                try:
                    ...
                    continue
                t = threading.Thread(target=self.__missing_logs, args=(recovery_stub, order_numbers))
                t.start()
                t.join()
                order_numbers = None
...
class RecoveryStub(object):
    ...
    def BackOnline(self):
        ...
        print('[RecoveryStub %d]' % self.stub_id, 'BackOnline(%d): %d' % (0, result.ping_number))
        return result.ping_number
    ...
    def RequestMissingLogs(self, order_numbers):
        ...
        for response in self.stub.RequestMissingLogs(self.missing_number_iterator(order_numbers)):
            ...
            print('[RecoveryStub %d]' % self.stub_id, 'RequestMissingLogs(%d): (%d, %s, %d)' %
            ...
            self.order_servicer.order_number = max(response.order_number + 1, self.order_servicer.order_number)
...
class RecoveryServicer(order2_pb2_grpc.RecoveryServicer):
    ...
    def BackOnline(self, request, context):
        ...
        print('[RecoveryServicer]', 'BackOnline(%d): %d' % (request.ping_number, max_number))
        return message
